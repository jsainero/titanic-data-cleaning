...
Las métricas obtenidas de la matriz de confusión son buenas, sobretodo la sensibilidad = 88.35% que corresponde a los pasajeros que el modelo ha clasificado como que no sobrevivieron respecto al total de pasajeros que no sobrevivieron.
...
Random Forest es implementa mediante la función _randomForest_ del paquete con el mismo nombre.
...
set.seed(666)
split <- floor(nrow(titanic)*0.75)

train2 <- titanic[1:split,]
test2 <- titanic[(split+1):nrow(titanic),]

train2 <- train2[,c(1:4,7)]
test2x <- test2[,c(2:4,7)]
test2y <- test2[,1]

# Modelo
model2 <- randomForest(Survived~., data=train2, ntree=1500)
model2
```

La tasa del error al crear el modelo es del 16.37%, concretamente del 7.37% para los no supervivientes y del 30.50 para los supervivientes.

Al hacer el plot del modelo se obtiene una gráfica el error para la clase 1 (rojo), la clase 2 (verde) y las muestras a usar (negro) sobre la cantidad de los 1500 árboles creados gracias al parámetro _ntree_.

```{r message= FALSE, warning=FALSE}
plot(model2)
```

Número de nodos usados en cada árbol: 
...
Se había probado un modelo inicial con todas las variables pero se descartaron aquellas con una importancia inferior a 20.

```{r message= FALSE, warning=FALSE}
predicted_model2 <- predict(model2, test2)
confusionMatrix(data=predicted_model2, reference=test2$Survived, positive='No')
```

Este modelo obtiene una _accuracy_ del 87.89% y una sensibilidad del 95.77%, unas métricas superiores a las obtenidas con el modelo de regresión logística.
